...
# Dictionary of specialized meta descriptions (110~140 chars)
$customDescriptions = @{
...
foreach ($file in $allFiles) {
    ...
    $cleanTitle = $rawTitle -replace '\s*-\s*云轨导航', ''
    $titleTagVal = if ($isRoot -and $filename -eq "index.html") { $rawTitle } else { "$cleanTitle - 云轨导航" }
    ...
    if ($customDescriptions.ContainsKey($filename)) {
        ...
        if ($existingDesc.Length -ge 105 -and $existingDesc.Length -le 160) {
            ...
            if ($filename.StartsWith("tutorial-")) {
                $finalDesc = "【官方客户端教程】$cleanTitle。详细讲解软件下载安装、订阅链接一键导入、分流规则配置及常见连接超时故障排除，助您在 Windows/Mac/iOS/Android 设备上轻松科学上网。"
            } elseif ($filename.StartsWith("beginner-")) {
                $finalDesc = "【新手必看指南】$cleanTitle。零基础全方位了解专线机场选择技巧、常见代理协议区别、客户端使用入门及晚高峰防卡顿防封锁秘籍，快速掌握科学上网核心技巧。"
            } elseif ($filename.StartsWith("nav-")) {
                $finalDesc = "【导航与资源推荐】$cleanTitle。精选优质 Telegram 电报频道、常用网络测试工具、流媒体检测脚本与安全防封导航，为您提供一站式跨境网络资源指引。"
                ...
                $finalDesc = "【专线机场深度测评】$cleanTitle。实测节点晚高峰网速与 Ping 延迟、IPLC 内网专线稳定性、Netflix 奈飞与 ChatGPT 原生 IP 解锁能力，提供真实性价比分析与套餐选购建议。"
            } else {
                $finalDesc = "关于 $cleanTitle 的深度解析与技术指南。由云轨导航编辑团队撰写，提供专业的 IPLC 专线测试、代理协议原理分析、客户端故障排查及极速稳定科学上网最佳实践。"
            }
        }
    }
...
"@
    ...
    if ($filename.StartsWith("tutorial-")) { $catName = "客户端教程" }
    elseif ($filename.StartsWith("beginner-")) { $catName = "新手指南" }
    elseif ($filename -in @("edgenova.html", "guangnianti.html", "huanyuyun.html", "jilianyun.html", "kexinyun.html", "kuaili.html", "shunyun.html", "sujie.html")) {
        $catName = "机场测评中心"
...
"@
    ...
    # 7. GEO Citation Widget for Articles
    if (-not $isRoot -and $filename.EndsWith(".html") -and $filename -ne "left.html") {
        $content = [System.Text.RegularExpressions.Regex]::Replace($content, '<div\s+class=["'']geo-cite-card["''].*?</div>\s*<script>.*?</script>', '', ([System.Text.RegularExpressions.RegexOptions]"Singleline, IgnoreCase"))
...
<div class="geo-cite-card" id="geo-cite-widget">
  ...
  <div class="geo-cite-box">
    <input type="text" class="geo-cite-input" id="geoCiteInput" readonly value="[$cleanTitle]($canonicalUrl)">
...
"@
    ...
    $processedCount++
    Write-Host "Processed [$relPath] -> Meta desc len: $($finalDesc.Length) chars."
}

Write-Host "Successfully processed $processedCount HTML files!" -ForegroundColor Green
