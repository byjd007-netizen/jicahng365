...
recommend_html = """      <!-- Recommended Links -->
...
for rev in reviews:
    ...
    if os.path.exists(path):
        ...
        content = re.sub(r'<!-- Recommended Links -->.*?</div>\s*</div>', recommend_html, content, flags=re.DOTALL)
        content = re.sub(r'<div class="related-recommendations".*?</ul>\s*</div>', recommend_html, content, flags=re.DOTALL)
...
content = re.sub(r'<!-- SEO Links Section -->.*?</section>', seo_footer, content, flags=re.DOTALL)
